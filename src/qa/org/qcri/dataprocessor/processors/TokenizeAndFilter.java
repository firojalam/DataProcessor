 ...
 * @author firojalam
 */
public class TokenizeAndFilter {
    private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(TokenizeAndFilter.class);
...
//            ));
    ...
    static String prolongChar = "(\\w)\\1\\1+";
    ...
    private static List<String> simpleTokenize(String text) {
        ...
        ArrayList<List<String>> bads = new ArrayList<List<String>>();	//linked list?
        ArrayList<Pair<Integer, Integer>> badSpans = new ArrayList<Pair<Integer, Integer>>();
        while (matches.find()) {
        ...
        ArrayList<List<String>> splitGoods = new ArrayList<List<String>>(indices.size() / 2);
     ...
     */
    public String tokenizeText(String text) throws IOException {

        String output = "";
        text = text.toLowerCase(); // lowercase
        text = Whitespace.matcher(text).replaceAll(" ").trim(); //remove multiple whitespace
        String[] arrText = text.split("\\s+");

        //// pattern to match and tag
        Pattern pURL = Pattern.compile(url);
        Pattern pEmail = Pattern.compile(Email);
        Pattern pAtMention = Pattern.compile(AtMention);
        Pattern pProlongChar = Pattern.compile(prolongChar);
        //// Emoticons: normalEyes, wink, noseArea, happyMouths, sadMouths, tongue, otherMouths
        Pattern pnormalEyes = Pattern.compile(normalEyes);
        Pattern pWink = Pattern.compile(wink);
        Pattern pNoseArea = Pattern.compile(noseArea);
        Pattern pHappyMouths = Pattern.compile(happyMouths);
        Pattern pSadMouths = Pattern.compile(sadMouths);
        Pattern pTongue = Pattern.compile(tongue);
        Pattern pOtherMouths = Pattern.compile(otherMouths);

        //Pattern pHashtag = Pattern.compile(Hashtag);
        for (int i = 0; i < arrText.length; i++) {
            try {
                String token = arrText[i].trim();
                String tag = "";
                Matcher mProlongChar = pProlongChar.matcher(token);
                if (pURL.matcher(token).find()) {
                    tag = "#URL";
                    output = output ;//+ " " + tag;
                } else if (StringUtils.isNumeric(token)) {
                    tag = "#DIGIT";
                    output = output ;//+ " " + tag;
                } else if (pEmail.matcher(token).find()) {
                    tag = "#EMAIL";
                    output = output ;//+ " " + tag;
                } else if (pAtMention.matcher(token).find()) {
                    tag = "#UserID";
                    output = output ;//+ " " + tag;
                } 
//            else if (pnormalEyes.matcher(token).find()) {
                //                tag = "#normalEyes#";
                //                output = output + " " + tag;
                //            }else if (pWink.matcher(token).find()) {
                //                tag = "#Wink#";
                //                output = output + " " + tag;
                //            }else if (pNoseArea.matcher(token).find()) {
                //                tag = "#NoseArea#";
                //                output = output + " " + tag;
                //            }else if (pHappyMouths.matcher(token).find()) {
                //                tag = "#HappyMouths#";
                //                output = output + " " + tag;
                //            }else if (pSadMouths.matcher(token).find()) {
                //                tag = "#SadMouths#";
                //                output = output + " " + tag;
                //            }else if (pTongue.matcher(token).find()) {
                //                tag = "#Tongue#";
                //                output = output + " " + tag;
                //            }else if (pOtherMouths.matcher(token).find()) {
                //                tag = "#OtherMouths#";
                //                output = output + " " + tag;
                //            }
                else if (mProlongChar.find()) {                    
                    String rep = mProlongChar.group(1) + mProlongChar.group(1);
                    StringBuffer sb = new StringBuffer();
                    mProlongChar.appendReplacement(sb, rep);
                    mProlongChar.appendTail(sb);
                    tag = sb.toString();
                    output = output + " " + tag;
                } else {
                    output = output + " " + token;
                ...
                ex.printStackTrace();
                LOG.error("Error in tokenizing..");
            }
        }
        output=output.replaceAll("[@$%^&*()_+-=\\!{\\}\\[\\]:\"|\\'\\~`<>/,\\\\]", "");
        List<String> toks = tokenizeRawTweetText(output);
        output="";
        for (String tok : toks) {
            output = output + " "+tok;
        }
        //output = String.join(" ", toks);
